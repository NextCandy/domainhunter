...
async function queryDb<T extends import("pg").QueryResultRow = import("pg").QueryResultRow>(
  sql: string,
  params?: unknown[],
) {
...
async function lookupDomainServer(domain: string) {
  const { lookupDomain } = await import("./rdap.server");
  return lookupDomain(domain);
}

async function fetchDnsServer(domain: string) {
  const { fetchDns } = await import("./enrich.server");
  return fetchDns(domain);
}

async function fetchArchiveServer(domain: string) {
  const { fetchArchive } = await import("./enrich.server");
  return fetchArchive(domain);
}

async function sendNotificationServer(
  channels: { bark?: string; webhook?: string },
  title: string,
  body: string,
) {
  const { sendNotification } = await import("./enrich.server");
  return sendNotification(channels, title, body);
}
...
export const discoverFn = createServerFn({ method: "POST" })
  .validator((d: unknown) => DiscoverFilters.parse(d))
  .handler(async ({ data }) => {
    const sb = await sbAdmin();
    ...
    let filtered = (rows ?? []).map((r: any) => ({
      ...r,
      metrics: metricsByDomain.get(r.id) ?? null,
    }));
    if (data.regex) {
      try {
        const re = new RegExp(data.regex, "i");
        filtered = filtered.filter((r: any) => re.test(r.name));
      } catch {
        // Invalid user regex filters are ignored so the discover query can still return results.
      }
    }
    if (data.archiveYearMin != null)
      filtered = filtered.filter(
        (r: any) => (r.metrics?.archive_year ?? 0) >= data.archiveYearMin!,
      );
    if (data.backlinksMin != null)
      filtered = filtered.filter((r: any) => (r.metrics?.backlinks ?? 0) >= data.backlinksMin!);
...
async function refreshOneInternal(domain: string) {
  ...
  const weights = await getWeights();
  const lookup = await lookupDomainServer(parsed.domain);
  const status =
...
export const refreshDomainFn = createServerFn({ method: "POST" })
  .validator((d: unknown) => z.object({ domain: z.string().min(3).max(253) }).parse(d))
  .handler(async ({ data }) => refreshOneInternal(data.domain));

export const refreshBulkFn = createServerFn({ method: "POST" })
  .validator((d: unknown) => z.object({ domains: z.array(z.string()).max(500) }).parse(d))
  .handler(async ({ data }) => {
...
export const checkRelatedTldsFn = createServerFn({ method: "POST" })
  .validator((d: unknown) => z.object({ name: z.string().min(1).max(63) }).parse(d))
  .handler(async ({ data }) => {
    const tlds = ["com", "net", "org", "io", "ai", "co", "cc", "cn"];
    const results = await Promise.all(
      tlds.map(async (tld) => {
        try {
          const r = await lookupDomainServer(`${data.name}.${tld}`);
...
export const listWatchlistFn = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    const { getMockStore, toMockDomain } = await import("@/lib/mock-api-store");
    const store = getMockStore();
    return store.watchlist.map((item) => ({
      ...item,
      mode: "mock",
      domain: toMockDomain(item.domain),
...
export const toggleWatchFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const { getMockStore, nextMockId } = await import("@/lib/mock-api-store");
      const store = getMockStore();
      const existing = store.watchlist.find((item) => item.domain === parsed.domain);
      if (existing) {
        store.watchlist = store.watchlist.filter((item) => item.id !== existing.id);
        return { watching: false, mode: "mock" };
      }
      store.watchlist.unshift({
        ...
        status: "watching",
        tags: ["mock"],
        note: "Mock fallback: database unavailable.",
...
export const updateWatchlistFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const { getMockStore } = await import("@/lib/mock-api-store");
      const store = getMockStore();
      const item = store.watchlist.find((row) => row.id === data.id);
      if (item) Object.assign(item, data.patch);
...
export const removeWatchFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      const sb = await sbAdmin();
      const { error } = await sb.from("watchlist").delete().eq("id", data.id);
      if (error) throw new Error(error.message);
      return { ok: true };
    } catch {
      const { getMockStore } = await import("@/lib/mock-api-store");
      const store = getMockStore();
      store.watchlist = store.watchlist.filter((item) => item.id !== data.id);
      return { ok: true, mode: "mock" };
    }
  });
...
export const listMyDomainsFn = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    const { getMockStore } = await import("@/lib/mock-api-store");
    return getMockStore().portfolio.map((item) => ({ ...item, mode: "mock" }));
  }
});
...
export const upsertMyDomainFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const { getMockStore, nextMockId } = await import("@/lib/mock-api-store");
      const store = getMockStore();
      const existing = store.portfolio.find((item) => item.domain === parsed.domain);
      const row = {
      ...
      else store.portfolio.unshift(row);
      return { ok: true, mode: "mock" };
    }
  });
...
export const removeMyDomainFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      const sb = await sbAdmin();
      const { error } = await sb.from("my_domains").delete().eq("id", data.id);
      if (error) throw new Error(error.message);
      return { ok: true };
    } catch {
      const { getMockStore } = await import("@/lib/mock-api-store");
      const store = getMockStore();
      store.portfolio = store.portfolio.filter((item) => item.id !== data.id);
      return { ok: true, mode: "mock" };
    }
  });
...
export const deleteRegistrarFn = createServerFn({ method: "POST" })
  .validator((d: unknown) => z.object({ id: z.coerce.number().int() }).parse(d))
  .handler(async ({ data }) => {
    const sb = await sbAdmin();
...
export const enrichDomainFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [dns, arc] = await Promise.all([
      fetchDnsServer(parsed.domain),
      fetchArchiveServer(parsed.domain),
    ]);
...
export const sendTestNotificationFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const res = await sendNotificationServer(
...
export const liveScanFn = createServerFn({ method: "POST" })
  .validator((d: unknown) => LiveScanSchema.parse(d))
  .handler(async ({ data }) => {
    // 1) 决定要查询的「名字」候选集
