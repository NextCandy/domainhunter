...
const createSchema = z.object({
  ...
  concurrency: z.number().int().min(ENRICH_LIMITS.concurrency.min).max(ENRICH_LIMITS.concurrency.max).default(ENRICH_LIMITS.concurrency.default),
  qps: z.number().int().min(ENRICH_LIMITS.qps.min).max(ENRICH_LIMITS.qps.max).default(ENRICH_LIMITS.qps.default),
  cacheTtlSeconds: z.number().int().min(ENRICH_LIMITS.cacheTtl.min).max(ENRICH_LIMITS.cacheTtl.max).default(ENRICH_LIMITS.cacheTtl.default),
});
...
export const advanceEnrichJobFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    for (let i = 0; i < pending.length; i++) {
      const it = pending[i];
      const r = results[i];
      let status: string, result: any = null, error: string | null = null;
      if (r.status === "fulfilled") {
        const v = r.value;
        status = v.cached ? "cached" : v.skipped ? "skipped" : "done";
        result = v.payload;
        if (v.cached) cached++; else done++;
      } else {
        status = "error";
...
async function processOne(it: { domain: string; kind: Kind }, ttl: number): Promise<{ cached?: boolean; skipped?: boolean; payload: any }> {
  ...
  if (it.kind === "dns") {
    ...
    if (r == null) { skipped = true; payload = { skipped: "semrush_unavailable" }; }
    else payload = r;
  }
...
export async function maybeAutoEnrich(sourceJobId: string) {
  ...
  if (!ej) return;
  const rows = domains.flatMap(d => kinds.map(k => ({
