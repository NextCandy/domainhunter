...
function AdminSources() {
  ...
  const importMut = useMutation({
    mutationFn: () =>
      importDomainsFn({ data: { source: sourceName, text, autoCheck, status: importStatus } }),
    onSuccess: (r) => {
      toast.success(`已导入 ${r.inserted} 个域名（解析 ${r.parsed}）`);
      setText("");
    },
  ...
  const upsertSrc = useMutation({
    mutationFn: (f: { name: string; type: string; url?: string }) =>
      upsertSourceFn({ data: { ...f, enabled: true, sync_interval_min: 1440 } }),
    onSuccess: () => {
      ...
      <section className="card-elev p-5">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              if (!srcName) return;
              upsertSrc.mutate({ name: srcName, type: "url", url: srcUrl || undefined });
              setSrcName("");
              setSrcUrl("");
            }}
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {s.type} · {s.url ?? "—"} · 同步 {s.sync_interval_min}m
                </div>
              </div>
              <button
                onClick={() => deleteSrc.mutate(s.id)}
