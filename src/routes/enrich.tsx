...
function EnrichListPage() {
  ...
  return (
    <AppShell>
      <PageHeader
        title="丰富抓取任务"
        description="DNS / Archive / SEO 批量抓取，带缓存与断点续查"
        actions={
          <button onClick={() => setShowForm((v) => !v)} className="btn-base btn-primary">
      ...
      {showForm && (
        <div className="card-elev mb-6 space-y-3 p-4">
          ...
          <textarea
            ...
            placeholder="域名列表，一行一个 / 用空格或逗号分隔"
            value={domainsText}
          ...
          <p className="text-xs text-muted-foreground">
            SEO 抓取需要 Semrush 连接器；未配置时该类型会被自动跳过并写入短期缓存。
          </p>
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(false)} className="btn-base btn-ghost">
              取消
            </button>
          </div>
        </div>
                ...
                <tr key={j.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 min-w-[200px]">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground tabular-nums">
                      {processed.toLocaleString()} / {j.total.toLocaleString()} · ✓{j.done} · 🗄
                      {j.cached_hits} · ✕{j.failed}
                    </div>
                  </td>
                  <td className="px-3 py-2 text-right">
                    <Link to="/enrich/$id" params={{ id: j.id }} className="btn-base btn-ghost">
...
export function StatusPill({ s }: { s: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ${cls[s] ?? "bg-accent"}`}
    >
      {s}
    </span>
  );
