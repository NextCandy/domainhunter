...
function AdminPricing() {
  ...
  const prices = useQuery({
    queryKey: ["prices", tldFilter],
    queryFn: () => listPricesFn({ data: { tld: tldFilter || undefined } }),
  });
  ...
  const savePrice = useMutation({
    ...
    onSuccess: () => {
      ...
      setPf({ ...pf, tld: "", register_price: "", renew_price: "", transfer_price: "", notes: "" });
    },
  ...
  const [cf, setCf] = useState({
    ...
    description: "",
    tlds: "",
    discount_type: "percent" as "percent" | "fixed" | "price",
    discount_value: "",
    valid_until: "",
    verified: true,
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-2">
        <button
          ...
          className={`rounded-md px-3 py-1.5 text-sm ${tab === "prices" ? "bg-primary text-primary-foreground" : "bg-surface ring-1 ring-border"}`}
        >
          价格表
        </button>
        <button
          onClick={() => setTab("coupons")}
          className={`rounded-md px-3 py-1.5 text-sm ${tab === "coupons" ? "bg-primary text-primary-foreground" : "bg-surface ring-1 ring-border"}`}
        >
          优惠码
        </button>
      ...
      <p className="text-xs text-muted-foreground">
        提示：在注册商管理中给启用的注册商配置 <code>config_json.prices_url</code>（可选{" "}
        <code>auth_header</code>/<code>auth_value</code>），定时任务每小时自动同步。
      </p>
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_1.6fr]">
          <section className="card-elev p-5">
            ...
            <div className="space-y-2 text-sm">
              <select
                value={pf.registrar_id}
                onChange={(e) => setPf({ ...pf, registrar_id: Number(e.target.value) })}
                className="field"
              >
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  placeholder="TLD (com)"
                  value={pf.tld}
                  onChange={(e) => setPf({ ...pf, tld: e.target.value })}
                  className="field"
                />
                <input
                  placeholder="货币 USD"
              ...
              <div className="grid grid-cols-3 gap-2">
                <input
                  placeholder="注册价"
                  value={pf.register_price}
                  onChange={(e) => setPf({ ...pf, register_price: e.target.value })}
                  className="field"
                />
                <input
                  placeholder="续费价"
                  value={pf.renew_price}
                  onChange={(e) => setPf({ ...pf, renew_price: e.target.value })}
                  className="field"
                />
                <input
                  placeholder="转入价"
                  value={pf.transfer_price}
                  onChange={(e) => setPf({ ...pf, transfer_price: e.target.value })}
                  className="field"
                />
              </div>
              <div className="flex flex-wrap gap-3 text-xs">
                <label className="flex items-center gap-1">
                  <input
                    type="checkbox"
                    checked={pf.privacy_free}
                    onChange={(e) => setPf({ ...pf, privacy_free: e.target.checked })}
                  />
                  免费隐私
                </label>
                <label className="flex items-center gap-1">
                  <input
                    type="checkbox"
                    checked={pf.api_supported}
                    onChange={(e) => setPf({ ...pf, api_supported: e.target.checked })}
                  />
                  API 可用
                </label>
              </div>
              ...
              />
              <button
                disabled={!pf.registrar_id || !pf.tld || savePrice.isPending}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="bg-muted/40 uppercase text-muted-foreground">
                  <tr>
                    <th className="px-3 py-2 text-left">注册商</th>
                    <th className="px-3 py-2">TLD</th>
                    <th className="px-3 py-2 text-right">注册</th>
                    <th className="px-3 py-2 text-right">续费</th>
      ...
      {tab === "coupons" && (
        <div className="grid gap-5 lg:grid-cols-[1fr_1.6fr]">
          <section className="card-elev p-5">
            ...
            <div className="space-y-2 text-sm">
              <select
                value={cf.registrar_id}
                onChange={(e) => setCf({ ...cf, registrar_id: Number(e.target.value) })}
                className="field"
              >
                <option value={0}>选择注册商…</option>
                {regOpts.map((r) => (
              ...
              />
              <input
                placeholder="适用 TLD (逗号分隔, 留空=全部)"
                value={cf.tlds}
                onChange={(e) => setCf({ ...cf, tlds: e.target.value })}
                className="field"
              />
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={cf.discount_type}
                  onChange={(e) => setCf({ ...cf, discount_type: e.target.value as any })}
                  className="field"
                >
                ...
                <input
                  ...
                  onChange={(e) => setCf({ ...cf, discount_value: e.target.value })}
                  className="field"
                />
              </div>
              <input
                type="date"
                value={cf.valid_until}
                onChange={(e) => setCf({ ...cf, valid_until: e.target.value })}
                className="field"
              />
              <div className="flex flex-wrap gap-3 text-xs">
                <label className="flex items-center gap-1">
                  <input
                    type="checkbox"
                    checked={cf.verified}
                    onChange={(e) => setCf({ ...cf, verified: e.target.checked })}
                  />
                  已验证
                </label>
                <select
                  value={cf.status}
                  onChange={(e) => setCf({ ...cf, status: e.target.value as any })}
              ...
              <button
                disabled={!cf.registrar_id || !cf.code || saveCoupon.isPending}
                ...
                <tbody>
                  {(coupons.data ?? []).map((c: any) => (
                    <tr key={c.id} className="border-t border-border/50">
                      ...
                      <td className="px-3 py-1.5 text-right">
                        <button onClick={() => delCoupon.mutate(c.id)} className="text-destructive">
                          <Trash2 className="inline h-3 w-3" />
                        </button>
                      </td>
                    </tr>
