...
import {
  ...
  stopEnrichJobFn,
  resumeEnrichJobFn,
} from "@/lib/enrich-jobs.functions";
...
function EnrichDetailPage() {
  ...
  const stopMut = useMutation({
    ...
    onSuccess: () => {
      toast.success("已停止");
      qc.invalidateQueries({ queryKey: ["enrich-job", id] });
    },
  });
    ...
    <AppShell>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Link to="/enrich" className="btn-base btn-ghost">
              <ArrowLeft className="h-4 w-4" />
              返回
            </Link>
            {isRunning ? (
              <button onClick={() => stopMut.mutate()} className="btn-base btn-ghost">
                <Pause className="h-4 w-4" />
                停止
              </button>
            ) : (
              <button onClick={() => resumeMut.mutate()} className="btn-base btn-primary">
            ...
            <a
              href={`/api/public/enrich/${id}/download?kind=enriched_csv`}
            ...
            <a
              href={`/api/public/enrich/${id}/download?kind=enriched_json`}
              className="btn-base btn-ghost"
            >
              <Download className="h-4 w-4" />
              JSON
            ...
            <a
              href={`/api/public/enrich/${id}/download?kind=available_enriched_csv`}
              className="btn-base btn-ghost"
            >
              <Download className="h-4 w-4" />
              可用域名 CSV
      ...
      <div className="card-elev mb-6 space-y-3 p-5">
        <div className="flex items-center justify-between text-sm">
          <div>
            状态 <StatusPill s={j.status} />
          </div>
          <div className="tabular-nums text-muted-foreground">
            {processed.toLocaleString()} / {j.total.toLocaleString()} ({pct.toFixed(1)}%)
          </div>
        </div>
        <ProgressBar pct={pct} />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
              ...
              {data.recent.map((r: any) => (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="py-1 text-muted-foreground">
                    {r.attempted_at ? new Date(r.attempted_at).toLocaleTimeString() : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <Section title="错误明细">
          {data.errors.length === 0 ? (
            <p className="text-xs text-muted-foreground">暂无错误</p>
          ) : (
              ...
              {data.errors.map((e: any) => (
                ...
                >
                  <div className="font-mono">
                    {e.domain} <span className="text-muted-foreground">/ {e.kind}</span>
                  </div>
