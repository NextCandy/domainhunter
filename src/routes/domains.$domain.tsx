...
function DomainDetailPage() {
  ...
  const refreshMut = useMutation({
    ...
    onSuccess: (r) => {
      ...
      refetch();
      qc.invalidateQueries({ queryKey: ["domain-detail", domain] });
    },
  ...
  const watchMut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(r.watching ? "已加入观察列表" : "已从观察移除");
      refetch();
    },
  });
  const enrichMut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(
        `已抓取 DNS · A=${r.dns.a_records.length} NS=${r.dns.ns_records.length}${r.archive.archive_year ? ` · Archive ${r.archive.archive_year}` : ""}`,
      );
      refetch();
    },
    onError: (e: unknown) => toast.error(messageOf(e, "抓取失败")),
  });
    ...
    return (
      <AppShell>
        <EmptyState
          ...
          action={
            <button onClick={() => refreshMut.mutate()} className="btn-base btn-primary">
    ...
    <AppShell>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="btn-base btn-ghost"
            >
              <Sparkles className={`h-4 w-4 ${enrichMut.isPending ? "animate-pulse" : ""}`} />
              {enrichMut.isPending ? "抓取中…" : "丰富 DNS/Archive"}
            </button>
            <button
              onClick={() => watchMut.mutate()}
              className={`btn-base ${watching ? "btn-ghost" : "btn-primary"}`}
            >
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card title="基础信息">
          ...
          <KV
            k="上次检测"
            v={d.last_checked_at ? new Date(d.last_checked_at).toLocaleString() : "—"}
          />
        </Card>
        ...
        <Card title="WHOIS / RDAP">
          {w ? (
            <>
              ...
              <KV
                k="创建时间"
                v={w.created_date ? new Date(w.created_date).toLocaleString() : "—"}
              />
              <KV k="到期时间" v={w.expiry_date ? new Date(w.expiry_date).toLocaleString() : "—"} />
              <KV
                k="更新时间"
                v={w.updated_date ? new Date(w.updated_date).toLocaleString() : "—"}
              />
              ...
              <details className="mt-3">
                ...
                <pre className="mt-2 max-h-72 overflow-auto rounded bg-muted p-3 text-[11px]">
                  {JSON.stringify(w.raw_data, null, 2)}
                </pre>
              </details>
            </>
          ) : (
        ...
        <Card title="历史 / SEO">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Archive 数据来自 Wayback Machine 公共 API。Ahrefs / Majestic 等外链数据需付费接入。
          </p>
        </Card>
