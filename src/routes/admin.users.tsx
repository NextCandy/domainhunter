...
import { listUsersFn, grantAdminFn, revokeAdminFn, resetUserPasswordFn } from "@/lib/users.functions";
...
function AdminUsersPage() {
  ...
  const resetPassword = useMutation({
    mutationFn: () => resetUserPasswordFn({ data: { userId: resetTarget?.id, password: newPassword } }),
    onSuccess: () => {
      toast.success("密码已重置");
      setResetTarget(null);
      setNewPassword("");
  ...
  return (
          ...
          {(data ?? []).map((u: any) => (
            <tr key={u.id} className="border-t border-border">
              ...
              <td className="px-3 py-2 text-right">
                <button onClick={() => { setResetTarget(u); setNewPassword(""); }} className="btn-base btn-ghost mr-1 text-xs"><KeyRound className="h-3.5 w-3.5" />重置密码</button>
                {u.is_admin
    ...
    {resetTarget && (
      <div className="card-elev p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">正在为 {resetTarget.email ?? "该用户"} 设置新密码。保存后旧刷新令牌会失效。</p>
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          ...
          >
            <KeyRound className="h-4 w-4" />{resetPassword.isPending ? "重置中…" : "确认重置"}
          </button>
          <button onClick={() => { setResetTarget(null); setNewPassword(""); }} className="btn-base btn-ghost">取消</button>
        </div>
      </div>
    )}
    </div>
