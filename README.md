...
- **全列可点排序**：发现 / 已删除 / 待删除等列表的每个表头（评分、状态、长度、类型、外链、到期 / 删除、风险…）均可点击切换升 / 降序。
- **域名列表管理**：支持**多选批量删除**与**单条删除**（级联清理评分 / WHOIS / DNS / 观察记录）。
- **移动端适配**：桌面表格在手机上自动切换为卡片视图，导航折叠为汉堡菜单，列表附带排序快捷栏。
...
### 观察 / 我的域名 / 其它
- **观察列表**：实时展示每个域名的真实状态（可注册 / 已注册 / 待删除 / 宽限期 / 赎回期 / 不支持），含标签、备注、删除前提醒；支持文件批量导入与 Bark / Webhook 通知通道。
- **我的域名**：列出注册商、注册日期、到期日期、DNS（域名服务器）等信息，表头可点击排序；支持通过文件或注册商 API（Spaceship）批量导入。
- 拍卖、待删除、已删除等多视图。
